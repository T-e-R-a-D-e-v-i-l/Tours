...
let favorites = [] // массив избранных туров
...
async function renderFavorites(tours) {

    const favoritTour = document.getElementById("favorites");
    favoritTour.innerHTML = "";

    if (favorites.length === 0) {
         favoritTour.innerHTML = "в избранном ничего не найдено";
        return;
    } else {
        tours.forEach((tour) => {
            favoritTour.innerHTML += `           
     ...
     if (favorites.includes(tour.id)) {
        tours.forEach((tour) => {
            ...
            document.getElementById(`favoritToursBtn${tour.id}`).style.fill = "red-600";
            favorites.push(tours);
            saveToLocalStorage()
            });
        })
    } else {
